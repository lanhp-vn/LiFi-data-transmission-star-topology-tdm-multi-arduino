    ...
    with open(input_file, 'rb') as file:
        ...
        while arduino.isOpen():
            ...
            while isTransmitting:
                ...
                if transmit_time * (user_id - 1) <= elapsed_time <= transmit_time * (user_id - 1) + timeslot_duration:

                    if toAttachID == 0:
                        arduino.write(address.encode('utf-8'))

                    data = file.read(no_bit_read)  # Read a byte from the file
                    if not data:  # If no byte is read (end of file), exit the loop
                ...
                if transmit_time * (user_id - 1) + timeslot_duration < elapsed_time <= transmit_time * user_id:
                    if endTimeslot == True:
                        arduino.write(endSignal_timeslot.encode('utf-8'))
                    endTimeslot = False
